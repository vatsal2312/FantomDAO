...
contract FantomDAOBondStakeDepository is Ownable {
    ...
    address public immutable MAGIC; // intermediate reward token from treasury
    address public immutable sMAGIC; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints MAGIC when receives principle
    ...
    struct Bond {
        uint gonsPayout; // sMAGIC gons remaining to be paid
        uint MAGICPayout; //MAGIC amount at the moment of bond
    ...
    constructor ( 
        ...
        require( _MAGIC != address(0) );
        MAGIC = _MAGIC;
        require( _sMAGIC != address(0) );
        sMAGIC = _sMAGIC;
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 MAGIC ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) MAGIC
         */
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            IERC20( MAGIC ).safeTransfer( DAO, fee ); 
        }
        ...
        IERC20( MAGIC ).approve( stakingManager, payout );

        IStakingManager(stakingManager).stake( payout, address(this) );
        /* ---------------------------------------------------------- */
        
        uint stakeGons=ISMAGIC(sMAGIC).gonsForBalance(payout);

        // depositor info is stored
        _bondInfo[ _depositor ] = Bond({ 
            gonsPayout: _bondInfo[ _depositor ].gonsPayout.add( stakeGons ),
            MAGICPayout: _bondInfo[ _depositor ].MAGICPayout.add( payout ),
    ...
    function redeem( address _recipient, bool _stake) external returns ( uint ) {        
        ...
        uint _amount = ISMAGIC(sMAGIC).balanceForGons(info.gonsPayout);
        emit BondRedeemed( _recipient, _amount, 0 ); // emit bond data
        IERC20( sMAGIC ).transfer( _recipient, _amount ); // pay user everything due
        return _amount;
    }
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( MAGIC ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    /**
     *  @notice return bond info with latest sMAGIC balance calculated from gons
     *  @param _depositor address
     *  @return payout uint
     *  @return vesting uint
    ...
    function bondInfo(address _depositor) public view returns ( uint payout,uint vesting,uint lastBlock,uint pricePaid ) {
        ...
        payout=ISMAGIC(sMAGIC).balanceForGons(info.gonsPayout);
    ...
    /**
     *  @notice calculate current ratio of debt to MAGIC supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = IERC20( MAGIC ).totalSupply();
    ...
    /**
     *  @notice calculate amount of MAGIC available for claim by depositor
     *  @param _depositor address
     *  @return pendingPayout_ uint
     */
    function pendingPayoutFor( address _depositor ) external view returns ( uint pendingPayout_ ) {
        uint percentVested = percentVestedFor( _depositor );
        uint payout = ISMAGIC(sMAGIC).balanceForGons(_bondInfo[ _depositor ].gonsPayout);
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or MAGIC) to the DAO
     *  @return bool
     */
    function recoverLostToken( address _token ) external returns ( bool ) {
        require( _token != MAGIC );
        require( _token != sMAGIC );
